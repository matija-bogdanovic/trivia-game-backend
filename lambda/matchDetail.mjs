/**
 * ===========================================================================
 * matchDetail — POST /matches/detail
 * ===========================================================================
 * The full record behind one of the player's own history entries. Own
 * matches only — you have to have been at the table to read it.
 *
 ...
 *         "Resource": "arn:aws:dynamodb:eu-west-3:637423486388:table/Matches" }
 *     ]
 *   }
 *   Plus AWSLambdaBasicExecutionRole for CloudWatch logs.
 *
 * ── API GATEWAY ────────────────────────────────────────────────────────────
 *   Route/Method:  POST /matches/detail
 ...
 *   Request:  POST /matches/detail   { "matchId": "<id>" }
 *   Response: 200 { match: { match_id, lobbyId, roomName, code, playedAt,
 *                            durationMs, rounds, winner, winnerName, margin,
 *                            standings[], participants[] } }
 *             400 { message: "matchId required" }
 *             403 { message: "Not your match" }
 ...
 *
 * Ported from: matchDetailHandler in economy.ts + getMatch in game/matches.ts
 * ===========================================================================
 */
...
export const handler = async (event) => {
  ...
  try {
    const { matchId } = body;
    if (!matchId) return json(event, 400, { message: "matchId required" });

    const res = await ddb.send(
      new GetCommand({ TableName: MATCHES_TABLE, Key: { match_id: String(matchId) } })
    );
    const match = res.Item ?? null;
    if (!match) return json(event, 404, { message: "Match not found" });
    if (!match.participants.includes(username)) {
      return json(event, 403, { message: "Not your match" });
    }
    return json(event, 200, { match });
  } catch (err) {
