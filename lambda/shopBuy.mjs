/**
 * ===========================================================================
 * shopBuy — POST /shop/buy
 * ===========================================================================
 * Spend coins on a shop item (currently: 3 lobby credits for 100 coins).
 *
 ...
 *   Request:  POST /shop/buy   { "itemId": "credits3" }
 *   Response: 200 { credits, coins, ownedAvatars }
 *             400 { message: "itemId required" | "Unknown item" |
 *                            "Already owned" | "Not enough coins" }
 ...
 *
 * Ported from: shopBuyHandler + buyItem in src/server/apis/economy.ts / game/wallet.ts
 * ===========================================================================
 */
...
export const handler = async (event) => {
  ...
  try {
    const { itemId } = body;
    if (!itemId) return json(event, 400, { message: "itemId required" });

    const item = SHOP_ITEMS.find((i) => i.id === String(itemId));
    if (!item) return json(event, 400, { message: "Unknown item" });

    const wallet = await getWallet(username);
    if (item.kind === "avatar" && wallet.ownedAvatars.includes(item.value)) {
      return json(event, 400, { message: "Already owned" });
    }
    if (wallet.coins < item.cost) {
      return json(event, 400, { message: "Not enough coins" });
    }
